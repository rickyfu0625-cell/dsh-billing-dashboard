...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const PRICES_PATH = "/api/billing-dashboard/prices";
		...
		const STRINGS = {
			zh: {
				...
				unknownModels: "检测到非 DeepSeek 官方模型，消费按兜底价估算（可能不准）：",
				manualPricingTitle: "非官方模型定价",
				manualPricingNote: "以下模型没有 DeepSeek 官方价，按兜底价估算。可手动输入每 1M token 单价（输入 / 缓存命中 / 输出），估算会更准。",
				save: "保存",
				clear: "清除",
			...
			en: {
				...
				unknownModels: "Non-DeepSeek models detected — cost priced at the fallback rate (may be inaccurate):",
				manualPricingTitle: "Non-official model pricing",
				manualPricingNote: "These models have no official DeepSeek price and are estimated at the fallback rate. Enter a per-1M-token price (input / cache hit / output) for a better estimate.",
				save: "Save",
				clear: "Clear",
		...
		// ---- 非官方模型手动定价编辑器 --------------------------------
		function PriceEditor({ model, existing, onSave, onDelete, t }) {
			const [currency, setCurrency] = useState(existing && existing.currency === "USD" ? "USD" : "CNY");
			const [input, setInput] = useState(existing ? String(existing.input) : "");
			const [cacheRead, setCacheRead] = useState(existing ? String(existing.cacheRead) : "");
			...
			const save = async () => {
				const i = parseFloat(input), c = parseFloat(cacheRead), o = parseFloat(output);
				if (!(i > 0 || c > 0 || o > 0)) return;
				setBusy(true);
				await onSave(model, { input: i || 0, cacheRead: c || 0, output: o || 0, currency });
				setBusy(false);
			};

			return jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 6, padding: "8px 0", borderTop: "1px solid var(--dsw-alias-border-l2)" }, children: [
				jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }, children: [
					jsx("span", { style: { fontSize: 12, fontWeight: 600, color: "var(--dsw-alias-label-primary)" }, children: model }),
					existing && jsx("button", { type: "button", onClick: () => onDelete(model), style: { border: 0, background: "transparent", color: "var(--dsw-alias-label-secondary)", fontSize: 11, cursor: "pointer" }, children: t("clear") })
				] }),
				jsxs("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }, children: [
					jsxs("label", { style: { display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11, color: "var(--dsw-alias-label-secondary)" }, children: [t("input"), jsx("input", { type: "number", min: 0, step: "0.0001", value: input, onChange: (e) => setInput(e.target.value), placeholder: "0", style: fieldStyle })] }),
					jsxs("label", { style: { display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11, color: "var(--dsw-alias-label-secondary)" }, children: [t("cacheHit"), jsx("input", { type: "number", min: 0, step: "0.0001", value: cacheRead, onChange: (e) => setCacheRead(e.target.value), placeholder: "0", style: fieldStyle })] }),
					jsxs("label", { style: { display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11, color: "var(--dsw-alias-label-secondary)" }, children: [t("output"), jsx("input", { type: "number", min: 0, step: "0.0001", value: output, onChange: (e) => setOutput(e.target.value), placeholder: "0", style: fieldStyle })] }),
					jsx("select", { value: currency, onChange: (e) => setCurrency(e.target.value), style: { ...fieldStyle, width: 52 }, children: [jsx("option", { value: "CNY", children: "¥" }), jsx("option", { value: "USD", children: "$" })] }),
					jsx("button", { type: "button", onClick: save, disabled: busy, style: { border: "1px solid var(--dsw-alias-accent-primary, #4f7cff)", background: "transparent", color: "var(--dsw-alias-accent-primary, #4f7cff)", borderRadius: 6, padding: "4px 10px", fontSize: 12, cursor: "pointer" }, children: busy ? "…" : t("save") })
				] })
			] });
		}
		...
		function useCurrentSession(sessions) {
			const [id, setId] = useState(() => (sessions && sessions.list && typeof sessions.list.getSnapshot === "function" ? sessions.list.getSnapshot().current : undefined));
			useEffect(() => {
				if (!sessions || !sessions.list || typeof sessions.list.subscribe !== "function") return undefined;
				const unsub = sessions.list.subscribe(() => setId(sessions.list.getSnapshot().current));
				return unsub;
			}, [sessions]);
			return id;
		}
		...
		// ---- 看板组件 ----------------------------------------------
		function makeDashboard(locale, sessions) {
			function Dashboard() {
				...
				const currentSessionId = useCurrentSession(sessions);
				...
				const load = useCallback(async (force) => {
					...
					try {
						const params = [];
						if (currentSessionId) params.push(`sessionId=${encodeURIComponent(currentSessionId)}`);
						if (force) params.push("force=1");
						const qs = params.length > 0 ? `?${params.join("&")}` : "";
						const res = await fetch(`${SUMMARY_PATH}${qs}`, { cache: "no-store" });
				...
				const scope = data && data.scope === "session" ? "session" : "global";
				...
				const manualPrices = data && data.manualPrices && typeof data.manualPrices === "object" ? data.manualPrices : {};

				const savePrice = async (model, values) => {
					try {
						const res = await fetch(PRICES_PATH, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model, ...values }) });
						const body = await res.json();
						if (body && body.ok && body.manualPrices) {
							setData((d) => (d ? { ...d, manualPrices: body.manualPrices } : d));
							load(true);
						}
					} catch {}
				};
				const deletePrice = async (model) => {
					try {
						const res = await fetch(PRICES_PATH, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model, delete: true }) });
						const body = await res.json();
						if (body && body.ok && body.manualPrices) {
							setData((d) => (d ? { ...d, manualPrices: body.manualPrices } : d));
							load(true);
						...
						open && jsxs("div", { ref: panelRef, style: { ...panelStyle, overflowY: "auto", ...computePanelStyle() }, children: [
							jsxs("div", { style: { ...rowStyle, padding: "12px 14px" }, children: [
								jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [
									...
									jsx("span", { style: { borderRadius: 999, padding: "1px 8px", fontSize: 11, fontWeight: 600, background: scope === "session" ? "color-mix(in srgb, var(--dsw-alias-accent-primary, #4f7cff) 16%, transparent)" : "var(--dsw-alias-bg-base, rgba(127,127,127,.08))", color: scope === "session" ? "var(--dsw-alias-accent-primary, #4f7cff)" : "var(--dsw-alias-label-secondary)" }, children: scope === "session" ? t("sessionScope") : t("globalScope") })
								] }),
							...
							models.length > 0
								? jsxs("div", { style: { padding: "12px 14px" }, children: [
									jsx("div", { style: { fontSize: 12, fontWeight: 600, marginBottom: 4 }, children: t("modelBilling") }),
									...models.map((m, i) => jsxs("div", { key: m.model, style: { padding: "7px 0", borderTop: i > 0 ? "1px solid var(--dsw-alias-border-l2)" : "none" }, children: [
										jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline" }, children: [
											jsx("span", { style: { fontSize: 12, fontWeight: 600, color: "var(--dsw-alias-label-primary)" }, children: m.model }),
											jsxs("span", { style: { ...value, fontSize: 13 }, children: [t("totalCost"), " ", formatCost(Number(m.totals.cost) || 0, currency)] })
										] }),
										jsxs("div", { style: { ...label, fontSize: 11, marginTop: 2, display: "flex", gap: 12, flexWrap: "wrap" }, children: [
											jsxs("span", { children: [t("todayCost"), " ", jsx("strong", { style: value, children: formatCost(Number(m.today.cost) || 0, currency) })] }),
											jsxs("span", { children: [t("todayTokens"), " ", jsx("strong", { style: value, children: formatTokens(dayTokens(m.today)) })] }),
											jsxs("span", { children: [t("totalTokens"), " ", jsx("strong", { style: value, children: formatTokens(dayTokens(m.totals)) })] })
										] })
									] }))
								] })
								: jsxs("div", { style: { padding: "12px 14px" }, children: [
									...
									jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline", marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--dsw-alias-border-l2)" }, children: [
										jsxs("span", { style: { ...label, fontSize: 11 }, children: [t("totalCost"), " · ", t("totalTokens")] }),
										jsxs("span", { style: value, children: [formatCost(totals ? Number(totals.cost) || 0 : 0, currency), " · ", formatTokens(dayTokens(totals))] })
									] })
								] }),
							...
							unknownModels.length === 0 && jsxs(Fragment, { children: [
								jsx("div", { style: divider }),
							...
							unknownModels.length > 0 && jsxs(Fragment, { children: [
								jsx("div", { style: divider }),
								jsxs("details", { style: { padding: "0 14px 10px" }, children: [
									jsx("summary", { style: { cursor: "pointer", color: "var(--dsw-alias-state-warn-primary)", fontSize: 12, fontWeight: 600, padding: "8px 0" }, children: `${t("manualPricingTitle")} (${unknownModels.length})` }),
									jsx("div", { style: { ...label, fontSize: 11, marginBottom: 2 }, children: t("manualPricingNote") }),
									...unknownModels.map((model) => jsx(PriceEditor, {
										key: model,
										model,
										existing: manualPrices[model],
										onSave: savePrice,
		...
		function apply(ctx) {
			...
			const Dashboard = makeDashboard(locale, sessions);
