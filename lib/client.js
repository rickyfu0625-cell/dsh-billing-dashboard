	...
	factory: (require) => {
		...
		const STRINGS = {
			zh: {
				...
				note: "余额为官方实时数据；消费优先取官方数据，否则按本地日志估算，非官方模型不计入消费。最终以平台账单为准。",
				togglePanel: "切换用量看板",
				close: "关闭",
				chartAria: "近 7 日消费趋势"
			},
			en: {
				...
				trendNoteOfficial: "Official daily cost",
				recharge: "Top up",
				usage: "Usage details",
				refresh: "Refresh",
				...
				note: "Balance is live official data; cost prefers official data, otherwise estimated from local logs; non-official models are excluded. Final billing is on the platform.",
		...
		function makeDashboard(locale) {
			function Dashboard() {
				...
				const costSource = offTodayCost !== null ? "official" : "estimate";

				const todayTokens = dayTokens(today);
				const todayCost = offTodayCost !== null ? offTodayCost : estCost(today);

				// 趋势：官方逐日消费覆盖估算值
				const offMap = {};
				if (official && Array.isArray(official.history)) {
				...
				return jsxs(Fragment, { children: [
						open && jsxs("div", { ref: panelRef, style: { ...panelStyle, overflowY: "auto", ...computePanelStyle() }, children: [
							...
							jsxs("div", { style: { padding: "12px 14px" }, children: [
								jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline" }, children: [
									jsxs("span", { style: { ...label, fontSize: 11, display: "inline-flex", alignItems: "center", gap: 5 }, children: [
										t("todayCost"),
										jsx("span", { style: { borderRadius: 4, padding: "0 5px", fontSize: 10, fontWeight: 600, background: costSource === "official" ? "color-mix(in srgb, var(--dsw-alias-state-success-primary) 16%, transparent)" : "var(--dsw-alias-bg-base, rgba(127,127,127,.08))", color: costSource === "official" ? "var(--dsw-alias-state-success-primary)" : "var(--dsw-alias-label-secondary)" }, children: costSource === "official" ? t("official") : t("estimate") })
									] }),
							...
							jsxs("div", { style: { padding: "12px 14px 6px" }, children: [
								...
								jsx(TrendChart, { series: trendSeries, currency, ariaLabel: t("chartAria"), lang })
							] }),
